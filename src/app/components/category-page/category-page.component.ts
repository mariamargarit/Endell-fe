...
export class CategoryPageComponent implements OnInit {
  ...
  getVariantBySubcategory(id: number | null) {
    this.variantService.getVariantsBySubcategory(id).subscribe(
      (res) => {
        console.log('res', res);
        this.variants = res;
  ...
  filterSet() {
    ...
    this.pageList = this.variants.filter((x: any) => {
      const { filterSize, filterColor } = this.filterForm.value;
      ...
      return (
        (filterSize && filterSize.length
          ? x.assignedValueDTOList.find((item: any) =>
              filterSize.includes(item.attributeValueId.val)
            )
          : true) &&
        (filterColor && filterColor.length
          ? x.assignedValueDTOList.find((item: any) =>
              filterColor.includes(item.attributeValueId.val)
            )
          : true)
      );
    });
  ...
  filterResults(text: string) {
    ...
    this.pageList = this.variants.filter((variant) =>
      variant?.productId?.name?.toLowerCase().includes(text.toLowerCase())
    );
  }
